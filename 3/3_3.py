...
def solving_problem(const_money):
		...
		for Storage in range (max_storage+1):
			...
			for Shipped in range (max_shipping+1):
				...
				if (0 <= Machines <= max_storage):
					if (Shipped > 0): cost_of_shipping = const_money + 10*Shipped
					else:
						Shipped = 0
						cost_of_shipping = 0
...
#	print ("Optimal plan is:")
	...
	print (save)
	for const_money in range (50,10000):
		...
		for i in range(len(new_withoult_costs)):
			if (new_withoult_costs[i] != save_withoult_costs[i]):
				...
				print ("Began optimal at " + str(const_money) + " const cost of shipping")
				break
		else: continue
		break 	
